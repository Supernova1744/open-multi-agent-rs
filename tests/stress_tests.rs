...
use open_multi_agent::{
    ...
    task::{queue::TaskQueue, scheduler::{Scheduler, SchedulingStrategy}},
...
async fn stress_concurrent_agent_pool_50_runs() {
    ...
    println!("[stress] 50 agent runs via pool(10): {} succeeded in {:?}", n, elapsed);
    assert_eq!(n, 50);
...
async fn stress_large_linear_task_queue_200() {
    ...
    let tasks: Vec<Task> = ids.iter().enumerate().map(|(i, id)| {
...
async fn stress_fan_out_task_graph() {
    ...
    let leaf_count = 50;
    let leaves: Vec<Task> = (0..leaf_count).map(|i| {
        let mut t = create_task(&format!("leaf-{}", i), "leaf", None, vec!["root".to_string()]);
        t.id = format!("leaf-{}", i);
        t
    }).collect();
...
async fn stress_cascade_failure_100_dependents() {
    ...
    let root = { let mut t = create_task("t0", "root", None, vec![]); t.id = "t0".to_string(); t };
    let dependents: Vec<Task> = (1..n).map(|i| {
    ...
    let failed_count = q.list().iter().filter(|t| t.status == TaskStatus::Failed).count();
    println!("[stress] cascade fail for 100 tasks in {:?}: {} failed", elapsed, failed_count);
...
async fn stress_rapid_queue_operations_1000() {
    ...
    let tasks: Vec<Task> = (0..1000).map(|i| {
    ...
    for i in 0..1000 {
        q.complete(&format!("t{}", i), Some(format!("result-{}", i))).unwrap();
    }
...
async fn stress_concurrent_memory_store_100() {
    ...
    println!("[stress] 100 concurrent memory store ops in {:?}: {} completed", elapsed, n);
    assert_eq!(n, 100);
...
async fn stress_shared_memory_concurrent_writes_50_agents() {
    ...
    println!("[stress] 50 concurrent shared memory writes in {:?}: {} entries", elapsed, all.len());
...
async fn stress_scheduler_500_tasks_10_agents() {
    let agents: Vec<AgentConfig> = (0..10).map(|i| agent_config(&format!("agent-{}", i))).collect();

    let tasks: Vec<Task> = (0..500).map(|i| {
        let mut t = create_task(&format!("task-{}", i), &format!("Do task {}", i), None, vec![]);
        t.id = format!("task-{}", i);
        t
    }).collect();
    ...
    assert_eq!(assignments.len(), 500);
    println!("[stress] CapabilityMatch schedule 500/10 in {:?}", elapsed_cm);
    ...
    assert_eq!(assignments.len(), 500);
    println!("[stress] DependencyFirst schedule 500/10 in {:?}", elapsed_df);

    for (name, elapsed) in [("RR", elapsed_rr), ("LB", elapsed_lb), ("CM", elapsed_cm), ("DF", elapsed_df)] {
        assert!(elapsed < Duration::from_secs(1), "{} took too long: {:?}", name, elapsed);
    }
}
...
async fn stress_topological_sort_complex_dag() {
    use open_multi_agent::task::topological_sort;
    ...
    println!("[stress] topological sort of {}-node layered DAG in {:?}", all_tasks.len(), elapsed);
...
fn stress_token_usage_accumulation_10k() {
    ...
    for _ in 0..10_000 {
        let chunk = TokenUsage { input_tokens: 100, output_tokens: 200 };
...
async fn stress_agent_20_sequential_runs_mock() {
    ...
    let mut success = 0;
    for i in 0..20 {
        let r = agent.run(&format!("Query {}", i), Arc::clone(&adapter)).await.unwrap();
        if r.success { success += 1; }
    }
    let elapsed = start.elapsed();
    println!("[stress] 20 sequential mock agent runs in {:?}: {}/20 succeeded", elapsed, success);
...
async fn stress_pool_burst_100_jobs() {
    ...
    println!("[stress] 100 burst jobs pool(5) in {:?}: peak={}, completed={}", elapsed, p, n);
    assert_eq!(n, 100);
...
async fn stress_30_concurrent_streams() {
    ...
    let handles: Vec<_> = (0..n).map(|i| {
        ...
        tokio::spawn(async move {
            ...
            let mut agent = Agent::new(
                AgentConfig { name: format!("bot-{}", i), model: "mock".to_string(), ..Default::default() },
                reg, exec,
            );
    ...
    println!("[stress] {} concurrent streams completed in {:?}", n_done, elapsed);
    assert_eq!(n_done, n);
    assert!(elapsed < Duration::from_secs(3));
}
...
async fn stress_trace_event_collection_50_agents() {
    ...
    impl Tool for NopTool {
        ...
        async fn execute(&self, _: &std::collections::HashMap<String, serde_json::Value>, _: &ToolUseContext) -> AgentResult<open_multi_agent::types::ToolResult> {
            Ok(open_multi_agent::types::ToolResult { data: "ok".to_string(), is_error: false })
        }
    }

    let total_events = Arc::new(AtomicUsize::new(0));
    let on_trace: open_multi_agent::types::OnTraceFn = {
    ...
    for i in 0..50 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let exec = Arc::new(ToolExecutor::new(Arc::clone(&reg)));
            let mut agent = Agent::new(
                AgentConfig { name: format!("tracer-{}", i), model: "mock".to_string(), ..Default::default() },
                reg, exec,
            );
    ...
    println!("[stress] 50 agents × 4 trace events = {} events collected in {:?}", count, elapsed);
...
async fn stress_message_bus_100_senders_10_receivers() {
    ...
    println!("[stress] MessageBus: 100 senders × 10 receivers = {} callbacks in {:?}", total_msgs, elapsed);
    assert_eq!(total_msgs, 100 * 10, "Expected {} callbacks, got {}", 100 * 10, total_msgs);
    assert!(elapsed < Duration::from_secs(5));
}
...
async fn stress_message_bus_broadcasts_50x20() {
    ...
    println!("[stress] 50 broadcasts × 20 subscribers = {} callbacks in {:?}", count, elapsed);
...
async fn stress_retry_100_tasks_each_with_2_retries() {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let result = execute_with_retry(
                move || {
                    ...
                    Box::pin(async move {
                        if n < 2 {
                            Err(open_multi_agent::error::AgentError::Other("fail".to_string()))
                        } else {
                            Ok(AgentRunResult {
                                success: true,
                                output: "ok".to_string(),
            ...
            if result.success { sc.fetch_add(1, Ordering::Relaxed); }
        }));
    }
    ...
    println!("[stress] 100 tasks × 3 attempts = {} calls, {} succeeded in {:?}", calls, success, elapsed);
    assert_eq!(calls, 300, "Expected 300 total calls (100 tasks × 3), got {}", calls);
...
fn stress_compute_retry_delay_10k() {
    ...
    let elapsed = start.elapsed();
    println!("[stress] 10k compute_retry_delay calls in {:?} (total={})", elapsed, total);
    assert!(elapsed < Duration::from_millis(10), "compute_retry_delay is too slow");
}
...
async fn stress_message_bus_concurrent_subscribe_unsubscribe() {
    ...
    println!("[stress] concurrent subscribe/unsubscribe: {} callbacks in {:?}", count, elapsed);
    // Each of 20 workers receives exactly 5 messages while subscribed
    assert_eq!(count, 20 * 5, "Expected {} callbacks, got {}", 20 * 5, count);
    assert!(elapsed < Duration::from_secs(5));
}
...
fn stress_skip_remaining_500_tasks() {
    ...
    let skipped = q.list().iter().filter(|t| t.status == TaskStatus::Skipped).count();
    println!("[stress] skip_remaining 500 tasks in {:?}: {} skipped", elapsed, skipped);
    assert_eq!(skipped, n);
